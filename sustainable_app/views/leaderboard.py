...
def leaderboard(request):
    ...
    users = list(User.objects.exclude(game_keeper=True).order_by(
        "username").values('id', 'username', 'xp', 'points'))
    users = add_users_level(users)
    equipped_items_by_username = User.objects.exclude(
        game_keeper=True
    ).order_by("username").prefetch_related('equipped_items')

    users = add_equipped_items(users, equipped_items_by_username)

    level = list(User.objects.exclude(game_keeper=True).order_by(
        "-xp").values('id', 'username', 'xp', 'points'))
    level = add_users_level(level)
    equipped_items_by_level = User.objects.exclude(
        game_keeper=True).order_by("-xp").prefetch_related('equipped_items')
    level = add_equipped_items(level, equipped_items_by_level)

    points = list(User.objects.exclude(game_keeper=True).order_by(
        "-points").values('id', 'username', 'xp', 'points'))
    points = add_users_level(points)
    equipped_items_by_points = User.objects.exclude(game_keeper=True).order_by(
        "-points").prefetch_related('equipped_items')
...
def add_equipped_items(users, user_equipped_items):
    ...
    for i, user in enumerate(users):
        equipped_items = list(
            user_equipped_items[i].equipped_items.values_list('type', 'name'))
        # Sets default
