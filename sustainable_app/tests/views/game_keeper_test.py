...
from sustainable_app.models import (
    User, Location, QuizQuestion, Survey,
    SurveyQuestion, SurveyChoice
)
...
class GameKeeperViewTests(TestCase):
    ...
    def test_call_view_as_user(self):
        """
        checks the game keeper page redirects to home when we are
        ...
        response = self.client.get(
            reverse('game_keeper_locations'), follow=True)
        self.assertRedirects(response, reverse('home'))
        ...
        # game keeper questions page
        response = self.client.get(
            reverse('game_keeper_questions'), follow=True)
        self.assertRedirects(response, reverse('home'))

    def test_call_view_as_game_keeper(self):
        """
        checks the game keeper page redirects to game_keeper when we are
        logged in as a game keeper
        """
        ...
        self.assertTemplateUsed(
            response, "sustainable_app/game_keeper_locations.html")

        # game keeper surveys page
        response = self.client.get(reverse('game_keeper_surveys'), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "sustainable_app/game_keeper_surveys.html")
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "sustainable_app/game_keeper_questions.html")
    ...
    def test_post_view_remove_game_keeper(self):
        ...
        new_keeper = User(username='test-username',
    ...
    def test_post_view_questions_add_and_remove(self):
        ...
        self.client.post(reverse('questions_remove'), {
            'question_id': QuizQuestion.objects.get(
                question='test-question').id,
        }, follow=True)

        self.assertFalse(QuizQuestion.objects.filter(
            question='test-question').exists())
    ...
    def test_post_view_surveys_add_and_remove(self):
        ...
        self.client.post(reverse('game_keeper_surveys'), {
            ...
            'o3': 'test-o3',
            'o4': 'test-o4',
            'survey_selection': Survey.objects.get(
                survey_text='test-survey'
            ).id,
            'form-type': 'create-question'
        }, follow=True)

        self.assertTrue(SurveyQuestion.objects.get(
            question_text='test-question'))
        self.assertTrue(SurveyChoice.objects.get(
            choice_text='test-o1',
            question=SurveyQuestion.objects.get(question_text='test-question')
        ))
        ...
        self.assertFalse(Survey.objects.filter(
            survey_text='test-survey').exists())
        self.assertFalse(SurveyQuestion.objects.filter(
            question_text='test-question').exists())
        self.assertFalse(SurveyChoice.objects.filter(
            choice_text='test-o1').exists())
